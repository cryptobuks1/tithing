...
class AdminProcess {
    ...
    function AdminProcess() {
        ...
        // Admin submitted delete banned user form 
            ...
            $this->assignment();
        } else if (isset($_POST['deassignment'])) {
            $this->deassignment();
        } else if (isset($_POST['remove_station'])) {
            $this->deleteStation();
        }
    ...
    function assignment() {
        ...
        $allocation_username = filter_input(INPUT_POST, 'allocation_username', FILTER_SANITIZE_STRING);
        $allocation_station = filter_input(INPUT_POST, 'allocation_station', FILTER_SANITIZE_STRING);
        $date = time();


        $database->assignment_data($allocation_username, $allocation_station, $date);

        header("Location: " . $session->referrer);
    }

    function deassignment() {
        global $database, $session;
        $deallocation_username = filter_input(INPUT_POST, 'deallocation_username', FILTER_SANITIZE_STRING);
        $deallocation_station = filter_input(INPUT_POST, 'deallocation_station', FILTER_SANITIZE_STRING);



        $database->deassignment_data($deallocation_username, $deallocation_station);
    ...
    function deleteStation() {
        global $database, $session;
        $delete_stations = filter_input(INPUT_POST, 'delete_stations', FILTER_SANITIZE_STRING);

        $database->delete_station($delete_stations);
    ...
    function addStation() {
        ...
        $station_location = filter_input(INPUT_POST, 'station_location', FILTER_SANITIZE_STRING);
        $station_region = filter_input(INPUT_POST, 'station_region', FILTER_SANITIZE_STRING) . " Region";


        $database->add_station($station_name, $station_region, $station_location);
    ...
    function setPrices() {
        ...
        $super_price = filter_input(INPUT_POST, 'super_price', FILTER_SANITIZE_STRING);
        $diesel_price = filter_input(INPUT_POST, 'diesel_price', FILTER_SANITIZE_STRING);
        $kero_price = filter_input(INPUT_POST, 'kero_price', FILTER_SANITIZE_STRING);
        $lpg_price = filter_input(INPUT_POST, 'lpg_price', FILTER_SANITIZE_STRING);
        if (empty($super_price)) {
            $super_price = '0.00';
        }
        if (empty($diesel_price)) {
            $diesel_price = '0.00';
        }
        if (empty($kero_price)) {
            $kero_price = '0.00';
        ...
        $database->update_price($diesel_price, $super_price, $kero_price, $lpg_price);
